...
+ No: 
  ...
  + Case-mix adjustment can exaggerate biases it tries to address: `r Citep(bib, "deeksEvaluatingNonrandomisedIntervention2003", .opts = list(max.names=2))`
...
__Key References:__
+ Original paper: `r Citep(bib, "jarmanExplainingDifferencesEnglish1999", .opts = list(max.names=2))`
+ Birmingham's criticism: `r Citep(bib, "mohammedEvidenceMethodologicalBias2009", .opts = list(max.names=2))` 
+ Paul Taylor's long-form article on history and controversy: `r Citep(bib, "taylorStandardizedMortalityRatios2014", .opts = list(max.names=2))`
...
+ Sheffield paper: `r Citep(bib, "campbellDevelopingSummaryHospital2012", .opts = list(max.names=2))`
...
+ They do not directly relate to quality of care`r Citep(bib,"lilfordUsingHospitalMortality2010","blackAssessingQualityHospitals2010a", .opts = list(max.names=2))`

+ They do not relate to avoidable death  `r Citep(bib, "girlingCasemixAdjustedHospital2012",  "hoganAvoidabilityHospitalDeaths2015", .opts = list(max.names=2))`
...
+ Case-mix adjustment fallacy `r Citep(bib, "mohammedEvidenceMethodologicalBias2009", opts = list(max.names=2))`
...
+ Originally used to visualise surgical outcome more intuitively `r Citep(bib,"lovegroveMonitoringPerformanceCardiac1999", .opts = list(max.names=2))`
+ Can actually add limits to plot using cusums `r Citep(bib,"sherlaw-johnsonMethodDetectingRuns2005", .opts = list(max.names=2))`
